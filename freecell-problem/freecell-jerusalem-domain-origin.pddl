...
(:action FROM-STACK-TO-MOVEABLE
  ...
  :effect
   (and (OCCUPIED) (ORSTACK) (not (CLEAR ?card)) (MOVEABLE ?card) (not (ON ?card ?card_prev)) (CLEAR ?card_prev))
)
...
(:action FROM-MOVEABLE-TO-FREE
  ...
  :effect
   (and (not (MOVEABLE ?card)) (not (ORSTACK)) (not (ORBOTT)) (not (OCCUPIED)) (INCELL ?card) (not (CELLSPACE ?nfree)) (CELLSPACE ?nfree_prev) )
)
...
(:action FROM-MOVEABLE-TO-BOTTOM
  ...
  :effect
   (and (not (MOVEABLE ?card)) (not (ORSTACK)) (not (ORFREE)) (not (OCCUPIED)) (CLEAR ?card)  (BOTTOMCOL ?card) (not (COLSPACE ?ncol)) (COLSPACE ?ncol_prev))
)
...
(:action FROM-MOVEABLE-TO-STACK
  :parameters (?card - card ?card_prev - card)
  :precondition
   (and (MOVEABLE ?card) (not (ORSTACK)) (CLEAR ?card_prev) (CANSTACK ?card ?card_prev))
  :effect
...
(:action FROM-MOVEABLE-TO-HOME
  ...
  :effect
   (and (not (MOVEABLE ?card)) (not (ORSTACK)) (not (ORBOTT)) (not (ORFREE)) (not (OCCUPIED))  (not (HOME ?card_to)) (HOME ?card))
)

; it moves a card from a column with more than one element to another column with at least a card 
; It can be applied only if does not exists a card that is movable
; it is used since it avoids that a card is moved to movable but then it returns to its initial state
