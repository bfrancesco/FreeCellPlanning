...
(:action FROM-STACK-TO-FREE-V-STACK
  :parameters (?card - card ?card_prev - card ?card_to - card  ?nfree - num ?nfree_prev - num)
  :precondition
   (and (CLEAR ?card) (ON ?card ?card_prev)
    (or
    ;STACK-TO-STACK 
    (and (CLEAR ?card_to) (CANSTACK ?card ?card_to))
    ;STACK-TO-FREE
    (and (CELLSPACE ?nfree) (SUCCESSOR ?nfree ?nfree_prev))
    )
    )
  :effect
    (and (not (ON ?card ?card_prev)) (CLEAR ?card_prev)
    (when
    ;STACK-TO-STACK
    (and (CLEAR ?card_to) (CANSTACK ?card ?card_to))
    (and (ON ?card ?card_to)  (not (CLEAR ?card_to))))
    
    (when
    ;STACK-TO-FREE
    (and (CELLSPACE ?nfree) (SUCCESSOR ?nfree ?nfree_prev) (not (and (CLEAR ?card_to) (CANSTACK ?card ?card_to))))
    (and (INCELL ?card) (not (CELLSPACE ?nfree)) (CELLSPACE ?nfree_prev) (not (CLEAR ?card)))
    )
    )
)
